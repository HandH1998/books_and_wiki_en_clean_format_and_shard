...
class Sharding:
    ...
    def segment_articles_into_sentences(self, segmenter):
        ...
        def chunks(data, size1, size2):
            it = iter(data)
            for i in range(0, 17000, size1):
                tmp_size=size1 if i+size1<=17000 else 17000-i
                # yield {k: data[k] for k in islice(it, size)}
                yield {k: data[k] for k in islice(it, tmp_size)}
            for i in range(17000, len(data), size2):
                tmp_size=size2 if i+size2<=len(data) else len(data)-i
                yield {k: data[k] for k in islice(it, tmp_size)}
        # def chunks1(data, size):
        #     it = iter(data)
        #     for i in range(17000, len(data), size):
        #         yield {k: data[k] for k in islice(it,i,min(i+size,len(data)))}
        if use_multiprocessing == 'manager':
            manager = multiprocessing.Manager()
            return_dict = manager.dict()
            jobs = []
            n_processes = 60   # in addition to the main process, total = n_proc+1

            def work(articles, return_dict):
                sentences = {}
                for article_id in articles.keys():
                # for i, article in enumerate(articles):
                    sentences[article_id] = segmenter.segment_string(articles[article_id])

                    if article_id % 5000 == 0:
                        print('Segmenting article', article_id)

                return_dict.update(sentences)
                print("dict size : {}".format(len(return_dict)))
            # pool=multiprocessing.Pool(processes=10)
            for item in chunks(self.articles, 200, 100000):
                # print(len(item))
                # pool.apply_async(work,args=(item,return_dict))
                ...
                jobs.append(p)
                p.start()
            # for item in chunks1(self.articles, 200000):
            #     p = multiprocessing.Process(target=work, args=(item, return_dict))
    ...
    def distribute_articles_over_shards(self):
        ...
        for file in self.output_training_files:
            for article_id in self.sentences:
                self.output_training_files[file].append(article_id)
    ...
    def write_single_shard(self, shard_name, shard):
        ...
        with open(shard_name, mode='w', newline='\n') as f:
            for article_id in shard:
                ...
                num +=1
                if(num%5000 == 0):
                     print('have written {} articles'.format(num))
